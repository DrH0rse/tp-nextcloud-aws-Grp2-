...
**Numéro d'équipe** : `GROUPE2`
**Nom de code de l'équipe** *(optionnel)* : `Team Rocket`
...
|------------|--------------|-------|---------------|
| `Thomas VIOLETTE` | Platform Lead (Rôle 1) | `prenom.nom@domaine.fr` | `@ghuser` |
| `Corentin EYDOUX` | Network Engineer (Rôle 2) | | |
| `Arnaud VIGIER` | Compute Engineer (Rôle 3) | | |
| `Jonathan JOLY` | Data Engineer (Rôle 4) | | |
| `Anelyse RIVIERE ` | Security Engineer (Rôle 5) | | |
...
flowchart TB
    ...
    subgraph vpc[VPC 10.30.0.0/16 — eu-west-3]
        subgraph public[Subnets publics 3a + 3b]
            alb[ALB HTTPS 443 self-signed]
            nat[NAT Gateway single-AZ]
        end

        subgraph app[Subnets privés app 3a + 3b]
            asg[ASG single EC2 Docker + Nextcloud]
        end

        subgraph db[Subnets privés DB 3a + 3b]
    ...
    subgraph aws[Services AWS régionaux]
        s3_primary[S3 primary storage KMS]
        s3_logs[S3 ALB logs 30j KMS]
        secrets[Secrets Manager db + admin]
        kms[KMS CMK rotation annuelle]
    end

    user --> dns --> alb --> asg
    asg --> rds
    asg -->|IAM role| vpce_s3 --> s3_primary
    asg -->|IAM role| vpce_secrets --> secrets
...
- [x] `Thomas VIOLETTE` — certifie l'exactitude des informations ci-dessus
- [x] `Corentin EYDOUX` — certifie l'exactitude des informations ci-dessus
- [x] `Arnaud VIGIER` — certifie l'exactitude des informations ci-dessus
- [x] `Jonathan JOLY` — certifie l'exactitude des informations ci-dessus
- [x] `Anelyse RIVIERE` — certifie l'exactitude des informations ci-dessus
